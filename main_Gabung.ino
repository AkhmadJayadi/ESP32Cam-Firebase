...
void loop() {
  ReadDatabaseFirebase();
  Serial.println(intValue);
  Serial.println(floatValue);

  if(intValue == 3){
    new_image = true;
    WriteDatabaseFirebase(1, 40.78);
  }
...
void WriteDatabaseFirebase(int dataint, float datafloat){
  if (Firebase.RTDB.setInt(&fbdo, "test/int", dataint)){
    ...
    if (Firebase.RTDB.setFloat(&fbdo, "test/float", datafloat)){
      Serial.println("PASSED");
      Serial.println("PATH: " + fbdo.dataPath());
      Serial.println("TYPE: " + fbdo.dataType());
    }
